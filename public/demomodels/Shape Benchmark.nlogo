globals [ num-shapes all-the-shapes ]
...
to setup
  ...
  cro 380 [
    if vary-shape? [
      set shape item (who mod length all-the-shapes) all-the-shapes
    ]
    if not vary-color? [ set color yellow ]
  ]
  reset-ticks
end

to go
  if ticks mod report-timer-every = 0 [
    show timer
    reset-timer
  ]
  ask turtles [
