    ...
    <div id="netlogo-ui-controls" class="netlogo-display-vertical" aria-label="NetLogo Model Application">
      ...
      <div style="position: relative; width: {{width}}px; height: {{height}}px"
        ...
        {{#widgetObj:key}}
          ...
          {{# type === 'hnwButton'   }} <hnwButtonWidget  id="{{>widgetID}}" isEditing="{{isEditing}}" x="{{x}}" width="{{width}}" y="{{y}}" height="{{height}}" widget={{this}} isHNW="{{isHNW}}" ticksStarted="{{ticksStarted}}" procedures="{{metadata.procedures}}" isSpectator="{{metadata.isSpectator}}" /> {{/}}
