  ...
  pens =
    if penHeader is "PENS"
      penLines.map(
        (line) ->
          pengex = /^"(.*(?<!\\))" ([^ ]*) ([^ ]*) ([^ ]*) ([a-zA-Z]*) "(.*(?<!\\))" "(.*)"$/
          [_, display, intervalStr, modeStr, colorStr, sil, penSetup, penUpdate] = line.match(pengex)
          interval = parseFloat(intervalStr)
          mode     = parseInt(  modeStr)
          color    = parseFloat(colorStr)
          inLegend = parseBool(sil)
          { type: "pen", display, interval, mode, color, inLegend, setupCode: penSetup, updateCode: penUpdate }
      )
    else
      []
...
convertMainSlider = (left, top, right, bottom, bodyLines) ->
  ...
  { type: "hnwSlider", left, right, top, bottom, display, variable, min, max, 'default': default_, step, units, direction }
